...
const App = ({data}) => {
  ...
  const [rawData, setRaw] = useState(data);
  ...
  const sortData = data => {
    ...
    const buildingGroupsOrderedAsc = [
      ...
      // sort the objects in the array by asc properties
      .sort((a, b) => {
      ...
      // Sort the array of building in each buildingzone in 'asc' order
      buildingGroupsOrderedAsc.forEach((item, index) => {
      ...
      console.log(buildingGroupsOrderedAsc)
      // return buildingGroupsOrderedAsc;
      setSortData(buildingGroupsOrderedAsc);
  ...
  return (
    <div className="container">
      ...
      <h1>Index</h1>
      <hr></hr>
      {sortedData.length > 0 && sortedData.map((buildingzone, index) => (
        <React.Fragment key={index}>
